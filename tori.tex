...
\begin{theorem}\label{thm:def_groups}(\cite{gordon-hales:15a}*{\S 2.2.2})
 ...
 the following holds:
 \begin{enumerate}
 \item  for $z\in Z(F)$, the set ${\cG_z}(F)$ is the set of $F$-points of a connected reductive group
...
In order to prove the proposition, we need to start with  more fixed choices: namely, we have to include both the fixed choices needed to define the group and its parahoric indexing set as above, and also the fixed choice of a resolution of the co-character lattice of a maximally split maximal torus in that group, as in \S \ref{sub:fixedchoices}. 
...
\begin{prop}\label{prop:main}
 Let $(\Xi, \cF)$, $Z_\Xi\to Z_\Xi^\ast$ and $\cG\to Z_\Xi$ be as in Theorem \ref{thm:def_groups}, and let $Y\to X$ be a surjective map of $\Gamma$-modules satisfying the compatibility condition with $\Xi$ as in the preceding paragraph. 
 % and let $\ff\in \cF$. 
...
Now suppose $z\in Z_\Xi$. The element $z$ in particular defines a tower of field extensions $E/L/F$ with $L/F$ unramified, as in \S \ref{sub:def_tori}, a quasi-split group $\bG_{z^\ast}$, and a cocycle $\zeta$ that defines an inner twisting (over $F$) between the quasi-split form $\bG_{z^\ast}$ and $\bG_z$, which is an $L$-isomorphism.   
Then  
