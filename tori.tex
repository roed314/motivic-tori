...
and  the Galois action on the absolute root datum  (which we suppress from the notation). Finally, given $\Sigma$ with its Galois action, we also fix a finite set $\cF_\Sigma$ that encodes the set of faces of an alcove in the building of $\bG$, as in \cite{hales:transfert}*{\S 2.1}. %(this follows from the description of parahoric subgroups in \cite{gross:parahorics}).  
...
We recall that a \emph{motivic constant} is an element of the ring of constructible motivic functions on a point, i.e., of $A:=\Z[\lef, \lef^{-1}, \frac{1}{1-\lef^{-i}}, i>0]$, where $\lef$ is a formal symbol which specializes to $q$.
...
is irreducible and compact modulo center for $z \in Z(F)$ and $F \in \loc_M$.
