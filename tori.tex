...
to give formulas defining $\bT(F)$ and $\NerC(\ri_F)$ in this language, we need to parameterize the possible tori $\bT$.
...
of tori over $F$.  Finally, in Section \ref{sub:NerCdefinable} we show that $\NerC(\ri_F)$ is a definable subgroup of $\bT(F)$.
...
Here we prove that $G_{\ff}:=G_{x, 0}$ (with $x$ in the interior of the facet $\ff$)   is definable for  all facets $\ff$.  
It was previously shown in \cite{CGH-2} that for all \emph{positive} $r>0$ and all {optimal} points $x$ in the building, the Moy-Prasad filtration subgroups $G_{x, r}$ are definable; here we fill in the last missing case, namely, $r=0$ (note that when $r=0$, the group $G(F)_{x,0}$ depends only on the facet containing $x$; thus, we do not need to worry about optimal points).
...
the parahoric subgroup $G(F)_\ff$ associated with $\ff$ via Bruhat-Tits theory is definable.  
...
Then the parahoric $G(F)_\ff\subset G(F)$ arises in a definable family of definable sets. 
