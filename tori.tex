...
$d\mu_z$ on $\cG_z$, and a motivic function $c$ on $Z$ such that, for every $F\in \loc_M$,
\[
...
it has the canonical parahoric subgroup as in \cite{gross:97a}, associated with a special point $x$ in the building. 
By Proposition \ref{prop:main},  the equivalence class of $x$ has a representative $\ff\in \cF$, and the canonical parahoric 
...
$$ c^\ast_F(z^\ast)d\mu_{\bG_{z}(F)}^\can  =  c^\ast_F(z^\ast) \psi_{\zeta}^\ast \left(d\mu_{\bG_{z^\ast}(F)}^\can \right).$$ 
The measure $\psi_{\zeta}^\ast \left(d\mu_{\bG_{z^\ast}(F)}^\can \right)$ is motivic, as a pull-back of a motiivc measure under a definable map. 
