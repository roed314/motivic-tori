...
Thus, once and for all, we fix $\Gamma$, as an abstract finite group, with fixed enumeration of its elements, $\sigma_1, \dots, \sigma_m$, with $m=|\Gamma|$. We will also need to use the maximal unramified sub-extension of $L$.
There is an exact sequence of Galois groups, 
$$ 1\to \gal(L/L^{ur})\to \gal(L/F)\to \gal(L^{ur}/F)\to 1.$$
We fix the ramification index $e$, and make the convention that the elements
$\sigma_1, \dots, \sigma_e$ of $\Gamma$ constitute the group $\gal(L/L^{\ur})$, and that $\sigma_m$ projects to the Frobenius automorphism of $L^{\ur}$ over $F$. 
In order to encode the data of the extension tower $L/L^{\ur}/F$, we let $f=m/e$, and first introduce the parameters $b_0,\dots, b_{f-1}$ ranging over $F$, which serve as the coefficients of a polynomial whose root generates $L^{\ur}$. 
We identify $L^{ur}$ with   $F[x]/(p(x))$, where $p(x)=\sum_{i=1}^f b_i x^i$. 
